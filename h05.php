...
<body>
   ...
   echo "<h3>Tudrukud</h3>";
   $tudrukud = array("malviina","kalviina","palviina","regiina","terminaatoriina","pesumasiina","objektiina","milfiina");
   sort($tudrukud);
   foreach ($tudrukud as $nimi) {
    ...
    echo "<h3>Tudrukud II</h3>";
    echo "<h5>Esimesed 3 nime</h5>";
    for ($i = 0; $i < 3; $i++) {
        echo $tudrukud[$i] . "</br>";
    }
    ...
    $suvalineNimi = rand(0, count($tudrukud) - 1);
    echo $tudrukud[$suvalineNimi];
    ...
    $vinnid = array("1GKS1GKC8FR966658", "1FTEW1C87AK375821", "1G4GF5E30DF760067", "1FTEW1CW9AF114701", "WAUGGAFC8CN433989", "3G5DA03E83S704506", "4JGDA2EB0DA207570", 
    ...
    echo "autosid on ". count( $margid);

    #kontrolli, kas massiviid on ühepikkused
    $margidd = count($margid);
    $vinnidd = count($vinnid);
    
    if ($margidd == $vinnidd) {
        echo "<br>Mõlemad massiivid on sama pikkused.";
    } else {
        echo "Massiivid on erineva pikkusega.";
    }
   # leia eraldi Toyotade ja Audide arv nimekirjas
   $toyotad = array_count_values($margid)["Toyota"];
